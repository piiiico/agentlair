 ...
 *
 * A two-way email channel that routes inbound AgentLair emails into Claude Code
 * sessions, and lets Claude reply via the send_email tool.
 *
 ...
 *   POLL_INTERVAL_MS    — optional. Polling interval in ms. Default: 60000 (1 min)
 *   ALLOWED_SENDERS     — optional. Comma-separated list of allowed sender emails.
 *                         If set, only emails from these senders trigger events.
 */
...
const ALLOWED_SENDERS = process.env.ALLOWED_SENDERS
  ? new Set(process.env.ALLOWED_SENDERS.split(',').map(s => s.trim().toLowerCase()))
  : null

if (!API_KEY || !ADDRESS) {
  process.stderr.write(
    'ERROR: AGENTLAIR_API_KEY and AGENTLAIR_ADDRESS are required.\n',
  )
  process.exit(1)
}
...
const mcp = new Server(
  { name: 'agentlair-email', version: '1.0.0' },
  {
    capabilities: {
...
mcp.setRequestHandler(CallToolRequestSchema, async req => {
  if (req.params.name === 'send_email') {
    ...
    try {
      const result = await sendEmail(sender, to, subject, text)
      return {
        content: [
          {
            type: 'text',
...
async function poll() {
  try {
    ...
    for (const msg of messages) {
      ...
      // Sender gate: prevent prompt injection from unknown senders
      if (ALLOWED_SENDERS && !ALLOWED_SENDERS.has(sender)) {
        process.stderr.write(
          `[agentlair-email] Dropping message from ungated sender: ${msg.from}\n`,
        )
        seenIds.add(msg.message_id)
        continue
      }
