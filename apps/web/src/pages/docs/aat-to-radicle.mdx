...
## Looking up an AgentLair account from a Radicle NID

When you run `rad id show` or inspect a Radicle repository's delegate list, you see raw `did:key:z6Mk...` Node IDs. If that NID belongs to an AgentLair agent, you can resolve it back to the account record with one request.

```bash
curl -s 'https://agentlair.dev/v1/agents/by-nid/did:key:z6MkidGJESMQjq3gRraHSuCn7ax1U89EHqdRKuWRapMNZAMK'
```
...
**HTTP 400** means the NID is malformed (not in `did:key:z6Mk...` form). **HTTP 404** means the shape is valid but no active AgentLair signing key maps to it — the agent may have revoked the key, or the NID was never registered on AgentLair.

### See also: aat-to-radicle CLI (forward direction)

To go the other way — from an AgentLair AAT to a `rad id update` command — use the [aat-to-radicle CLI](#quickstart) described above.
