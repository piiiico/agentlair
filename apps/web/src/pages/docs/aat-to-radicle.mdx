...
Pipe an AAT into the tool. It verifies the JWKS signature, cross-checks `al_nid` against the agent's DID document, and prints the `rad id update` invocation.

```bash
# Issue an AAT for an agent with a registered signing key:
curl -s -X POST https://agentlair.dev/v1/tokens/issue \
  ...
  | jq -r .data.token \
  | bun https://raw.githubusercontent.com/piiiico/agentlair/main/tools/aat-to-radicle/aat-to-radicle.ts --format sh
```

Output: a single line of the form `rad id update --delegate did:key:z6Mk...`. Run it inside the Radicle working copy.
