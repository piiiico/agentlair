...
Not identity. Not authorization. Not even governance-within-a-single-organization. Trust — the continuous answer to the question: *is this agent behaving as it claims to, right now, in a way that can be verified by parties other than itself?*
...
But here is what neither the Framework nor the Toolkit addresses: **cross-organizational behavioral trust.** The Framework handles coordination. The Toolkit handles internal governance. Neither answers what happens when Agent X from Company A participates in a workflow with Company B, and Company B needs to know — continuously, not just at handshake time — whether Agent X is trustworthy.
...
**The scorer cannot be the scored.** Trust computation must happen on infrastructure the agent does not control. Not the agent's cloud account. Not the agent's organization's internal toolkit. A third party with its own incentive to compute accurately — because relying parties will switch providers if scores diverge from reality.
...
That number — 41 — is not impressive by design. It reflects a real agent with real behavioral variance, operating for weeks, building trust incrementally. A cold-start agent begins at 30 (Bayesian prior). Trust rises through demonstrated behavior over time. An agent that has operated for 90 days with consistent, restrained, transparent behavior might reach 72. An agent that games the system — producing artificially uniform signals — triggers an entropy penalty that caps its score at 85% of computed value.
...
- **Trust computation** happens on AgentLair infrastructure, not the agent's. Three dimensions — consistency (predictability), restraint (permission discipline), transparency (audit completeness) — are independently scored and weighted.
...
This is what MCP-I Level 4 looks like: behavioral trust that is continuous (not point-in-time), independent (not self-reported), multi-dimensional (not a single number), and manipulation-resistant (not gameable through artificial uniformity).
