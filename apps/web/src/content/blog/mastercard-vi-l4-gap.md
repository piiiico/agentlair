...
Mastercard shipped a real technical standard this month: an open-source specification (not a press release, not a whitepaper with no code) called **Verifiable Intent**, co-developed with Google, published at [verifiableintent.dev](https://verifiableintent.dev), with partners including Fiserv, IBM, and Checkout.com.
...
Read that again. Mastercard's own spec, co-authored with Google and backed by IBM, Fiserv, and Checkout.com, has a designated extension point for behavioral trust scoring. They built the hook. They named the gap. They did not fill it.
...
Mastercard is right to separate them. And the separation confirms, in a production-grade specification from the world's second-largest payment network, that behavioral trust is the acknowledged gap.

## The L3 Stack Is Now Complete

Step back and look at what shipped in the last six months:

- **Visa TAP** (Trusted Agent Protocol): answers "who is this agent?" via HTTP Message Signatures, JWKS-backed identity
- **Mastercard VI** (Verifiable Intent): answers "was this agent authorized by the cardholder?" via SD-JWT delegation chains
- **x402** (Linux Foundation): answers "how does this agent pay?" via HTTP-native payment transport, 22+ founding members
...
The artifact at [verifiableintent.dev](https://verifiableintent.dev) completes L3. The L4 artifact that computes whether an agent should be trusted, based on behavioral commitments verified against outcomes, doesn't exist yet.
