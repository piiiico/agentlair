...
export async function handleCalendarRoutes(
  ...
  if (path === '/v1/calendar/feed.ics' && method === 'GET') {
    ...
    const ical = renderICalendar(events, 'AgentLair Calendar');
  ...
  if (path === '/v1/calendar/events' && method === 'POST') {
    ...
    if (index.length >= limits.max_events) {
      return err(
        `Calendar event limit reached (${limits.max_events} events on ${account.tier || 'free'} tier). Delete old events or upgrade.`,
        403,
